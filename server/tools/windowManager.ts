...
`;
    const out = (await runPowerShell(script, 1500)).trim();
    if (out && out.includes("|")) {
 ...
 */
export async function focusBrowserWindow(browserInfo?: Partial<ActiveBrowserInfo>): Promise<boolean> {
  try {
    const pid = browserInfo?.pid || 0;
    const script = `
$proc = if (${pid} -ne 0) { Get-Process -Id ${pid} -ErrorAction SilentlyContinue } else { Get-Process -Name chrome, msedge, firefox, brave, opera, arc -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne [IntPtr]::Zero } | Select-Object -First 1 }
...
`;
    const out = (await runPowerShell(script, 2000)).trim();
    return out === "SUCCESS";
  } catch (err) {
