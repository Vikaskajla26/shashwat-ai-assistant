 ...
 * Exclusively queries google.com/search for live search snippets and Knowledge Cards
...
export async function fetchLiveSearchResults(query: string): Promise<{
  ...
  // Fetch from Google Search HTML Endpoint (Returns Google Featured Snippets & Knowledge Cards)
  try {
  ...
  // Compile Comprehensive Live Google Search Summary Text
  ...
  results.forEach((r, i) => {
    summaryParts.push(`[Google Live Result ${i + 1}] ${r.title}: ${r.snippet}`);
  });
